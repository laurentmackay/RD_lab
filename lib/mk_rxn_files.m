...
model_defs=model_defs(cellfun(@(x) ~isempty(x),model_defs));

if ~isempty(model_defs)
    non_unique = cellfun(@(x) length(x)>1,model_defs);
    cellfun(@(x) warning(['The model variable ' x ' has multiple definitions. Using the last one in the file.']) ,vars(non_unique));
    model_defs=cellfun(@(x) x{end},model_defs,'UniformOutput',false);
end
...
rhs=rhs(cellfun(@(x) ~isempty(x),rhs));

if ~isempty(rhs)
    non_unique = cellfun(@(x) length(x)>1,rhs);
    rhs=cellfun(@(x) x{end},rhs,'UniformOutput',false);
end
...
N_undetermined = N_chem-N_fast_eqn;
...
elim_fast={};
for i=1:size(p_elim_fast,1)
    e=p_elim_fast{i};
    inds =ismember(e,fast_chems);
    p_elim_fast{i}={e{inds} e{~inds}};
end
...
is_fast_elim = cellfun(@(x)any(strcmp(elim_fast,x)),chems);



p_elim = cellfun(@(pe) intersect(pe,chems(~is_fast_elim),'stable'),p_elim_con,'UniformOutput',0);
p_elim = p_elim_con;
...
if ~isempty(fast_chems)
    ...
    if length(elim_fast)==1
        elim_defs = cellstr(strcat(elim_fast{1},'=',string(sol_fast)));
...
fast_refs = strcat('(?<=(?:^|[',code,']))(',chems(is_fast_elim),')(?=(?:$|[',code,']))')';
consrv_rhs = regexprep(string(consrv_eqns),'^.*==','');
consrv_rhs_elim = regexprep(string(ec0),'^.*==','');



slow_init_reps = cellstr(string(init(~is_fast_elim)));
con_fast = cellfun(@(x) any(strcmp(x,elim_fast)),elim_con);
...
missing_init = ismissing(init(~is_fast_elim));
...
if N_fast_eqn>0
    ...
    init(is_fast_elim) = fast_init_reps;
else
...
chem_eqn = str2sym(chems(~is_fast_elim));
init_eqn = str2sym(init(~is_fast_elim));
...
%% COMPUTING J_GAMMA
J_gamma=arrayfun(@(gi) cellfun(@(j) simplify(diff(gi,j),'Steps',10),{chems{~is_fast_elim}},'UniformOutput',0),Gamma_0,'UniformOutput',false)';
if ~isempty(J_gamma)
    J_gamma = [J_gamma{:}];
    J_gamma = [J_gamma{:}];
    J_gamma=reshape(J_gamma,[N_undetermined,size(Gamma,1)])';
end
J_gamma_0 = J_gamma;
%
%
...
j_gam = sym('J_gamma_', [N_fast_eqn N_undetermined]);
if ~isempty(J_gamma)
    %     m0( J_gamma == 0 )=0;
...
i_non_fast = find(~is_fast_elim);
ind = is_slow&~is_fast_elim;

eye_slow_0= diag(ind,0);
eye_slow_0 = eye_slow_0(ind,~is_fast_elim);
...
i_0 = mod(find(A_li_rows(mask_slow_0,:)')-1,N_undetermined)'+1;
...
% f_slow={};
for chem_ind = find(~is_fast_elim)
...
is_elim_con = is_elim_con0(~is_fast_elim);

ode_body_elim = char(strjoin(strcat('d',chems(~is_elim_con0 & ~is_fast_elim),'/dt=',string(f_mix(~is_elim_con))'),newline));
...
chem_ref_elim=cellfun(@(x) ref(x),chems(~is_elim_con0 & ~is_fast_elim),'UniformOutput',0);
% chem_rep_elim=arrayfun(@(i) ['$<pre>x\(vox+' num2str(i-1) '*sz\)$<post>'],1:length(chems(~is_elim_con)),'UniformOutput',0);


%  fast_eqns = subs(fast_eqns, str2sym(elim_con'), consrv_subs);

chem_rep_FVM_elim=arrayfun(@(i) ['$<pre>u\(:,' num2str(i) '\)$<post>'],1:length(chems(~is_elim_con0 & ~is_fast_elim)),'UniformOutput',0);
...
init_sym = subs(subs(str2sym(init)), str2sym([chems(~is_fast_elim) chems(is_fast_elim)]), str2sym([slow_init_reps, fast_init_reps']));
...
if self_contained
    ...
    is_ode = ~is_elim_con0 & ~is_fast_elim;
    ...
    fp=init;
    fp(~is_elim_con0 & ~is_fast_elim)=num2str(ic_ode',12);
    ...
    fp(is_fast_elim) = regexprep(string(subs(cell2sym(sol_fast))),nameref(chems(~is_fast_elim)),fp(~is_fast_elim));
    fp(is_fast_elim) = regexprep(fp(is_fast_elim), par_refs, par_reps);
    ...
    consrv_eqn_fast = subs(consrv_eqns, [str2sym(elim_fast); consrv_nm_eqn], [cell2sym(sol_fast_0); consrv_val_eqn]);
