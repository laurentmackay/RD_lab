...
%             h_bndry_next =;
            ...
            M_(inds_next(end),inds_next)=[hmax-h_bndry_next h_bndry_next];
            
%                                 [-1 4 -5 2]/h^2
%             b_(inds_next(end))=sum((u_prev(inds,i_)+Rx(inds_half_next,i_)*dt).*h_local(inds));
            b_(inds_next(end))=sum(u_prev(inds,i_).*h_bndry'+Rx(inds_half_next,i_).*(h_bndry+h_bndry_next)'*dt/2);
%                                 b_(inds_next(end))=sum((u_prev(inds,i_)).*h_local(inds) );
...
% end
            ...
            h_bndry_half_next =[ h_bndry_half_next h-h_bndry_half_next];
...
%             b_(inds_next(1))=sum((u_prev(inds,i_)+Rx(inds_half_next,i_)*dt).*h_local(inds));
            b_(inds_next(1))=sum(u_prev(inds,i_).*h_bndry'+Rx(inds_half_next,i_).*(h_bndry+h_bndry_next)'*dt/2);
        end
        end
