...
%          h_local = repmat(h,N,1);
         ...
         i_local = ceil(bndry/h)+[0; 1];
         i_local_next = ceil(bndry_next/h)+[0; 1];
         
        if v_cell>0
         f1_star( i_local(1,:)+[-1 0],:)=0;
         f1_star( floor((a+P)/h)+1,:)=0;
         end
%          h_local(i_local)=h+rem(bndry-h/2,h).*[1 1; -1 -1];
%          h_local_next(i_local_next)=h+rem(bndry_next-h/2,h).*[1 1; -1 -1];
%           h_local(i_local)=h;
         h_local(i_local)=abs(bndry-x(i_local)-[-1 -1; 1 1]*h/2);
         h_local_next(i_local_next)=abs(bndry_next-x(i_local_next)-[-1 -1; 1 1]*h/2);
%          h_local(ceil(a/h)) = h+rem(a-h/2,h);
%          h_local(ceil(a/h)+1)=h-rem(a-h/2,h);
%          h_local(ceil((a+P)/h)) = h+rem((a+P)-h/2,h);
%          h_local(ceil((a+P)/h)+1)=h-rem((a+P)-h/2,h);
         
%          h_local_next(ceil(bndry_next(1)/h)) = h+rem((a+v_cell*dt)-h/2,h);
%          h_local_next(ceil(bndry_next(1)/h)+1)=h-rem((a+v_cell*dt)-h/2,h);
%          h_local_next(ceil(((a+v_cell*dt)+P)/h)) = h+rem(((a+v_cell*dt)+P)-h/2,h);
%          h_local_next(ceil(((a+v_cell*dt)+P)/h)+1)=h-rem(((a+v_cell*dt)+P)-h/2,h);
         for i_=1:N_species
             
             A_ = -(f1_star - f1_star(jump(:,2),:))*V(i_);
             u(:,i_)=(eye.*h_local_next-A_*dt/2)\((eye.*h_local+A_*dt/2)*u(:,i_)+Rx_star(:,i_)*dt.*h_local);
%             u(:,i_) = u(:,i_) + dudt{i_}*dt;
         end
%          h_local(floor(a/h)) = h;
%          h_local(floor(a/h)+1)=h;
%          h_local(floor((a+P)/h)) = h;
%          h_local(floor((a+P)/h)+1)=h;
%          h_local_next(floor((a+v_cell*dt)/h)) = h;
%          h_local_next(floor((a+v_cell*dt)/h)+1)=h;
%          h_local_next(floor(((a+v_cell*dt)+P)/h)) = h;
%          h_local_next(floor(((a+v_cell*dt)+P)/h)+1)= h;

         h_local(i_local)=h;
         h_local_next(i_local_next)=h;
